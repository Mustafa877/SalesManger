...
namespace MySystem
{
    public partial class OrderForm : Form
    {
        ...
        {   
            ...
            moduleForm.guna2AnimateWindow1.AnimationType = (Guna.UI2.WinForms.Guna2AnimateWindow.AnimateWindowType)Guna.UI.WinForms.GunaAnimateWindow.AnimateWindowType.AW_CENTER;
        ...
        private void dgvUser_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("هل انت متاكد من حذف هذا الطلب", "طلب حذف", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    con.Open();
                    cm = new SqlCommand("DELETE FROM tbOrder WHERE orderid LIKE '" + dgvOrder.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", con);
                    cm.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("تم  استرجاع الماده لمخزن ");
